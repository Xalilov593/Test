...
@section('content')
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        ...
        <div class="container mt-5">
            <h2 class="text-center mb-4">Foydalanuvchi Yaratish</h2>
            <form action="{{ route('users.store') }}" method="POST">
                @csrf

                <div class="mb-3">
                    <label for="name" class="form-label">Foydalanuvchi nomi</label>
                    <input type="text" class="form-control" id="username" name="name" placeholder="Ism kiriting" required>
                </div>

                <div class="mb-3">
                    <label for="email" class="form-label">Email</label>
                    <input type="email" class="form-control" id="email" name="email" placeholder="Email kiriting" required>
                </div>
